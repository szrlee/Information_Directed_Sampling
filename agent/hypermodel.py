...
class PriorHyperLinear(torch.nn.Module):
    def __init__(
        ...
        if isinstance(prior_std, np.ndarray):
            if prior_std.ndim == 1:
                ...
                prior_std = np.diag(prior_std, dtype=np.float32)
            ...
            assert isinstance(
                prior_std, (float, int, np.float32, np.int32, np.float64, np.int64)
            )
            prior_std = np.eye(output_size, dtype=np.float32) * prior_std

        self.weight = torch.nn.Parameter(
            torch.from_numpy(prior_std @ self.weight).float()
        )
    ...
    def forward(self, x):
        out = torch.nn.functional.linear(
            x, self.weight.to(x.device), self.bias.to(x.device)
        )
        return out
    ...
    def extra_repr(self):
        return "in_features={}, out_features={}, bias={}".format(
            self.in_features,
            self.out_features,
            not np.all(self.bias.cpu().detach().numpy() == 0),
        )
...
class HyperLinear(nn.Module):
    def __init__(
        ...
        self.priormodel = PriorHyperLinear(
            noise_dim, out_features, prior_mean, prior_std
        )
