...
now = datetime.now()
now_str = now.strftime("%Y_%m%d_%H%M_%S")
path = "../storage/test_allold/" + now_str
...
if check_Bernoulli:
    ...
    beta = exp.bernoulli_expe(
        T=200,
        n_expe=10,
        n_arms=3,
        methods=bernoulli_methods,
        param_dic=param,
        labels=labels,
        colors=colors,
        doplot=False,
